...
for line in sys.stdin:
    ...
    logging.info(f"LEN(LINE): {len(values)}")
    logging.info(f"LEN(OUTFIELDS): {len(outfields)}")
    logging.info(f"LINE: {values}")
    logging.info(f"OUTFIELDS: {outfields}")
    if (flag==1) and (len(values) < len(outfields)):
        outfields = [outfields[0]] + outfields[2:]
        fields = [fields[0]] + fields[2:]
    flag += 1

    click_record = dict(zip(fields, values))
    #apply filter conditions
    if filter_cond(click_record):
        logging.info(f"UserId: {click_record[outfields[0]]}")
        output = "\t".join([click_record[x] for x in outfields])
        print(output)

logging.info(f"line_count: {flag-1}")
